...
run-client:
	valgrind $(VALGRIND_FLAGS) ./lab_client ./file.txt $(THE_BUFFER_SIZE)

run-server:
	valgrind $(VALGRIND_FLAGS) ./lab_server ./file-recv.txt $(THE_BUFFER_SIZE)
	diff -qs ./file.txt ./file-recv.txt
